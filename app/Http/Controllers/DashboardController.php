    ...
    public function compare(Request $request)
{
    ...
    $dataType = $request->input('data_type', 'facturado'); // Valor por defecto 'facturado'
    $period1 = $request->input('period1');
    $period2 = $request->input('period2');
    
    $data = $this->getComparisonByType($type, $period1, $period2, $dataType);
    
    return response()->json($data);
}
    private function getComparisonData($year, $month, $dataType = 'facturado')
{
    $currentPeriod = RipsData::byPeriod($year, $month)->sum($dataType);
    $previousPeriod = RipsData::byPeriod($year, $month - 1)->sum($dataType);
    $previousYear = RipsData::byPeriod($year - 1, $month)->sum($dataType);
    ...
    private function getComparisonByType($type, $period1, $period2, $dataType)
{
    switch ($type) {
        case 'month':
            return $this->compareMonths($period1, $period2, $dataType);
        case 'quarter':
            return $this->compareQuarters($period1, $period2, $dataType);
        case 'year':
            return $this->compareYears($period1, $period2, $dataType);
        default:
            return [];
    }
}

    private function compareMonths($period1, $period2, $dataType)
{
    list($year1, $month1) = explode('-', $period1);
    list($year2, $month2) = explode('-', $period2);
    
    $data1 = RipsData::byPeriod($year1, $month1)
        ->selectRaw("regimen, SUM($dataType) as total")
        ->groupBy('regimen')
        ->get();
    ...
    private function compareQuarters($period1, $period2, $dataType)
{
    ...
    private function compareYears($year1, $year2, $dataType)
{
